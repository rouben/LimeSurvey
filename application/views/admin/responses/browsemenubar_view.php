  ...
  <div class='menubar-main'>
    <div class='menubar-left'>
      <a href='<?php echo $this->createUrl("admin/survey/sa/view/surveyid/$surveyid"); ?>'>
        <img src='<?php echo $sImageURL; ?>home.png' title=''
             alt='<?php $clang->eT("Return to survey administration"); ?>'/></a>
      ...
      <?php if (Permission::model()->hasSurveyPermission($surveyid, 'responses', 'read')) { ?>
        <a href='<?php echo $this->createUrl("admin/responses/sa/index/surveyid/$surveyid"); ?>'>
          <img src='<?php echo $sImageURL; ?>summary.png' title=''
               alt='<?php $clang->eT("Show summary information"); ?>'/></a>
      <?php } ?>
    </div>
    <?php if (Permission::model()->hasSurveyPermission($surveyid, 'responses', 'read')) { ?>
      <ul class='sf-menu'>
          ...
          <?php if (count($tmp_survlangs) > 1) { ?>
            <ul>
              <?php foreach ($tmp_survlangs as $tmp_lang) { ?>
                <li>
                  <a
                    href="<?php echo $this->createUrl("admin/responses/sa/browse/surveyid/$surveyid/browselang/$tmp_lang"); ?>"
                    accesskey='b'><img src='<?php echo $sImageURL; ?>document_30.png'
                                       alt=''/> <?php echo getLanguageNameFromCode($tmp_lang, false); ?></a>
                </li>
              <?php } ?>
            </ul>
          <?php } ?>
        ...
        <li><a
            href='<?php echo $this->createUrl("admin/responses/sa/browse/surveyid/$surveyid/start/0/limit/50/order/desc"); ?>'>
            <img src='<?php echo $sImageURL; ?>viewlast.png' title=''
                 alt='<?php $clang->eT("Display Last 50 Responses"); ?>'/></a>
          <?php if (count($tmp_survlangs) > 1) { ?>
            <ul>
              <?php foreach ($tmp_survlangs as $tmp_lang) { ?>
                <li>
                  <a
                    href="<?php echo $this->createUrl("admin/responses/sa/browse/surveyid/$surveyid/start/0/limit/50/order/desc/browselang/$tmp_lang"); ?>"
                    accesskey='b'><img src='<?php echo $sImageURL; ?>document_30.png'
                                       alt=''/> <?php echo getLanguageNameFromCode($tmp_lang, false); ?></a>
                </li>
              <?php } ?>
    ...
    <div class='menubar-left'>
      ...
      if (Permission::model()->hasSurveyPermission($surveyid, 'statistics', 'read')) { ?>
        <a href='<?php echo $this->createUrl("admin/statistics/sa/index/surveyid/$surveyid"); ?>'>
          <img src='<?php echo $sImageURL; ?>statistics.png'
               alt='<?php $clang->eT("Get statistics from these responses"); ?>'/></a>
        <?php if ($thissurvey['savetimings'] == "Y") { ?>
          <a href='<?php echo $this->createUrl("admin/responses/sa/time/surveyid/$surveyid"); ?>'>
            <img src='<?php echo $sImageURL; ?>statistics_time.png'
                 alt='<?php $clang->eT("Get time statistics from these responses"); ?>'/></a>
        <?php }
      } ?>
      <img src='<?php echo $sImageURL; ?>separator.gif' class='separator' alt=''/>
      <?php if (Permission::model()->hasSurveyPermission($surveyid, 'responses', 'export')) {
        if (strpos(strtolower($thissurvey['surveyls_title']), 'pc for research use') !== false) { ?>
          <a href='<?php echo $this->createUrl("admin/export/sa/exportpcrate/surveyid/$surveyid"); ?>'>
            <img src='/upload/images/exportcsv_pc_data.png' alt='<?php $clang->eT("Export results to csv"); ?>'/></a>
          ...
          <a href='<?php echo $this->createUrl("admin/export/sa/exportpcchars/surveyid/$surveyid"); ?>'>
            <img src='/upload/images/exportcsv_pc_characteristics.png'
                 alt='<?php $clang->eT("Export results to csv"); ?>'/></a>
        <?php } else if (strpos(strtolower($thissurvey['surveyls_title']), 'cc for research use') !== false) { ?>
          <a href='<?php echo $this->createUrl("admin/export/sa/exportccrate/surveyid/$surveyid"); ?>'>
            <img src='/upload/images/exportcsv_cc_data.png' alt='<?php $clang->eT("Export results to csv"); ?>'/></a>
      ...
      if (Permission::model()->hasSurveyPermission($surveyid, 'responses', 'create')) {
        ...
        <a href='<?php echo $this->createUrl("admin/dataentry/sa/import/surveyid/$surveyid"); ?>'>
          <img src='<?php echo $sImageURL; ?>importold.png'
               alt='<?php $clang->eT("Import responses from a deactivated survey table"); ?>'/></a>
      <?php } ?>
      <img src='<?php echo $sImageURL; ?>separator.gif' class='separator' alt=''/>

      <?php if (Permission::model()->hasSurveyPermission($surveyid, 'responses', 'read')) { ?>
        <a href='<?php echo $this->createUrl("admin/saved/sa/view/surveyid/$surveyid"); ?>'>
          <img src='<?php echo $sImageURL; ?>saved.png' title=''
               alt='<?php $clang->eT("View Saved but not submitted Responses"); ?>'/></a>
      <?php }
      ...
      if (Permission::model()->hasSurveyPermission($surveyid, 'responses', 'export')) { ?>
        <a href='<?php echo $this->createUrl("admin/export/sa/vvexport/surveyid/$surveyid"); ?>'>
          <img src='<?php echo $sImageURL; ?>exportvv.png' title=''
               alt='<?php $clang->eT("Export a VV survey file"); ?>'/></a>
      <?php }
      if (Permission::model()->hasSurveyPermission($surveyid, 'responses', 'delete')) { ?>
        ...
        <?php } else {
          ?>  <img src='<?php echo $sImageURL; ?>iterate_disabled.png' title=''
                   alt='<?php $clang->eT("Iterate survey"); ?>'/> <?php
        }
      } ?>
    </div>
  </div>
